...
impl Graph {
    pub async fn append(
        ...
        fn queue_resolve(
            send: async_channel::Sender<JoinHandle<color_eyre::Result<()>>>,
        ...
        while let Ok(f) = recv.recv().await {
            f.await??;
        }
    ...
    pub fn resolve_req(
        ...
        Ok(self
            ...
            .with_suggestion(|| "Make sure that the lockfile is up-to-date. Passing --immutable prevents any changes to the lockfile.")?
            .clone())
    }
