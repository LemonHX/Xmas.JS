...
use crate::utils::encoding::Encoder;
use crate::utils::{
    object::ObjectExt,
...
use rsquickjs::{Ctx, Error, Exception, FromJs, Null, Object, Result, Value};
use tokio::fs::File;
...
mod tests {
    use crate::buffer;
    use crate::utils::test::{call_test, call_test_err, test_async_with, ModuleEvaluator};
    use rsquickjs::{CatchResultExt, CaughtError};
    ...
    async fn given_file(content: &str, options: &mut OpenOptions) -> (File, PathBuf) {
        // Create file
        let path = crate::utils::test::given_file(content).await;
