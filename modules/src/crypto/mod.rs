...
use crate::utils::bytes::{ERROR_MSG_ARRAY_BUFFER_DETACHED, ERROR_MSG_NOT_ARRAY_BUFFER};
use crate::utils::ctx::CtxExtension;
use crate::utils::encoding::{bytes_to_b64_string, bytes_to_hex_string};
use crate::utils::{
...
pub static SYSTEM_RANDOM: LazyLock<SystemRandom> = LazyLock::new(SystemRandom::new);

fn encoded_bytes<'js>(ctx: Ctx<'js>, bytes: &[u8], encoding: &str) -> Result<Value<'js>> {
    match encoding {
        "hex" => {
            let hex = bytes_to_hex_string(bytes);
            let hex = rsquickjs::String::from_str(ctx, &hex)?;
            Ok(Value::from_string(hex))
        }
        "base64" => {
            let b64 = bytes_to_b64_string(bytes);
            let b64 = rsquickjs::String::from_str(ctx, &b64)?;
