...
use crate::http::agent::Agent;
use crate::http::client::HyperClient;
use crate::utils::encoding::bytes_from_b64;
use crate::utils::{
    ...
    globals.set(
        ...
        Func::from(Async(move |ctx, resource, args| {
            ...
            async move {
                ...
                let mut abort_receiver = options.abort_receiver;
...
fn build_request(
    ...
    if !detected_headers.contains("user-agent") {
        // 谁叫我家用户喜欢写爬虫捏~
        req = req.header("user-agent", "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_10) Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/500.50 (KHTML, like Gecko) Chrome/150.0.0.0 Safari/500.50");
    }
...
struct FetchOptions<'js> {
    ...
    abort_receiver: Option<crate::utils::mc_oneshot::Receiver<Value<'js>>>,
    redirect: String,
...
fn get_fetch_options<'js>(
    ...
    if let Some(obj) = resource.as_object() {
        let obj = obj.clone();
        if obj.instance_of::<super::Request>() {
...
mod tests {
    ...
    use crate::http::HttpsModule;
    use crate::utils::test::{call_test, test_async_with, ModuleEvaluator};
    use rsquickjs::{prelude::Promise, CatchResultExt};
    ...
    async fn test_fetch_function() {
        ...
        crate::utils::compression::zstd::encoder(welcome_message.as_bytes(), 3)
            .unwrap()
        ...
        let mut data: Vec<u8> = Vec::new();
        crate::utils::compression::brotli::encoder(welcome_message.as_bytes())
            .read_to_end(&mut data)
            .unwrap();
        ...
        let mut data: Vec<u8> = Vec::new();
        crate::utils::compression::gz::encoder(
            welcome_message.as_bytes(),
            crate::utils::compression::gz::Compression::default(),
        )
        .read_to_end(&mut data)
        .unwrap();
        ...
        let mut data: Vec<u8> = Vec::new();
        crate::utils::compression::zlib::encoder(
            welcome_message.as_bytes(),
            crate::utils::compression::zlib::Compression::default(),
        )
        .read_to_end(&mut data)
        .unwrap();
