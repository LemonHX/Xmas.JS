...
static NODE_MODULES_PATHS_CACHE: LazyLock<Mutex<NodeModulePaths>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));

static HOME_NODE_MODULES: LazyLock<Vec<Box<str>>> = LazyLock::new(|| {
    // Add global folders
...
static FILESYSTEM_ROOT: LazyLock<Box<str>> = LazyLock::new(|| {
    #[cfg(unix)]
    {
        "/".into()
    }
...
pub fn require_resolve<'a>(
    ...
    if x.starts_with('#') {
        ...
        if let Ok(Some(path)) = load_package_imports(ctx, x, &dirname_y) {
            info!("⛄🥕 Resolved by `LOAD_PACKAGE_IMPORTS`: {}", path);
            return Ok(path.into());
        }
    }
            ...
            if Path::new(&current_file).is_file() {
                ...
                match find_the_closest_package_scope(&x) {
                    ...
                    None => {
                        ...
                        info!("❄️  load_as_file(2.b.1): {}", current_file);
                    ...
                    Some(path) => {
                        ...
                        if let Some(_type) = get_string_field(&package_json, "type") {
                            ...
                            if _type == "module" || _type == "commonjs" {
                                info!("❄️  load_as_file(2.c.[1|2]): {}", current_file);
                                return Ok(Some(current_file.into()));
                            }
                        }
                    }
                ...
                info!("❄️  load_as_file(2.d): {}", current_file);
                return Ok(Some(current_file.into()));
            }
                ...
                match find_the_closest_package_scope(&x) {
                    ...
                    Some(path) => {
                        ...
                        if let Some(_type) = get_string_field(&package_json, "type") {
                            ...
                            if _type == "module" {
                                info!("❄️  load_index(1.c.1): {}", file);
                                return Ok(Some(file.into()));
                            }
                        }
                        // 2. Else, load X/index.js as an CommonJS module. STOP.
                        info!("❄️  load_index(1.c.2): {}", file);
                        return Ok(Some(file.into()));
                    }
                }
            }
...
fn load_as_directory<'a>(ctx: &Ctx<'_>, x: Rc<String>) -> Result<Option<Cow<'a, str>>> {
    ...
    if Path::new(&file).is_file() {
        ...
        if let Some(main) = get_string_field(&package_json, "main") {
            ...
            if let Ok(Some(path)) = load_as_file(ctx, m.clone()) {
                info!("❄️  load_as_directory(1.d): {}", path);
                return Ok(Some(path));
            }
            // e. LOAD_INDEX(M)
            if let Ok(Some(path)) = load_index(ctx, m) {
                info!("❄️  load_as_directory(1.e): {}", path);
                return Ok(Some(path));
            }
    ...
    if let Ok(Some(path)) = load_index(ctx, x) {
        info!("❄️  load_as_directory(2): {}", path);
        return Ok(Some(path));
    }
    ...
    if let BorrowedValue::Object(map) = package_json {
        ...
        if let Some(BorrowedValue::Object(exports)) = map.get("exports") {
            if let Some(BorrowedValue::Object(name)) = exports.get(modules_name) {
                ...
                if let Some(BorrowedValue::Object(platform)) = name.get(NODE) {
                ...
                if let Some(BorrowedValue::String(platform)) = name.get(NODE) {
        ...
        if let Some(BorrowedValue::String(platform)) = map.get(NODE) {
            return Ok((platform.as_ref(), None, is_cjs));
        }
